...
static void RB_SurfaceSpriteFixed (void)
{
	...
	RB_AddQuadStamp(re->origin, up, left, re->shaderRGBA);
}
...
static void RB_SurfaceBeamQ3mme (void)
{
	...
	for (i = 0;  i < repetitions;  i++) {
		...
		RB_AddQuadStamp(origin, left, up, re->shaderRGBA);
...
** RB_SurfaceRailCore
*/
static void RB_SurfaceRailCore (qboolean useRadius, qboolean nudgeOrigin)
{
	refEntity_t *e;
	int			len;
	vec3_t		right;
	...
	if (useRadius) {
		...
		size = r_railCoreWidth->integer;
		//DoRailCore(start, end, right, len, r_railCoreWidth->integer);
	}
	//if (e->angle) {
	if (e->rotation) {
		//for ( i = 0 ; i < 4 ; i++ ) {
		for ( i = 0 ; i < (180 / e->rotation) ; i++ ) {
			vec3_t	temp;

			//DoRailCore( start, end, right, len, 8 );
			DoRailCore( start, end, right, len, size );
			RotatePointAroundVector( temp, vec, right, e->rotation );
...
#if 0
static void RB_SurfaceRailCoreQ3mme (qboolean useRadius)
{
	refEntity_t *e;
	int			len;
	vec3_t		right;
	...
	//VectorCopy( e->origin, end );
	VectorCopy(e->origin, start);
	VectorMA(e->origin, VectorLength(e->oldorigin), e->oldorigin, end);
...
static void RB_SurfaceEntity( surfaceType_t *surfType ) {
	...
	case RT_BEAM:
		...
		break;
	case RT_RAIL_CORE:
		RB_SurfaceRailCore(qfalse, qfalse);
		//RB_SurfaceRailCore(qfalse, qtrue);
		break;
	case RT_RAIL_RINGS:
		RB_SurfaceRailRings();
		break;
