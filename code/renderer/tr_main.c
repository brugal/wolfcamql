...
void R_SetupProjectionZ(viewParms_t *dest)
{
	...
	// matrix for RF_DEPTHHACK

	memcpy(dest->projectionMatrixDepthHack, dest->projectionMatrix, sizeof(dest->projectionMatrixDepthHack));

	zNear	= r_znear->value;
	zFar	= 65000.0;  //FIXME
	depth	= zFar - zNear;

	dest->projectionMatrixDepthHack[2] = 0;
	dest->projectionMatrixDepthHack[6] = 0;
	dest->projectionMatrixDepthHack[10] = -( zFar + zNear ) / depth;
	dest->projectionMatrixDepthHack[14] = -2 * zFar * zNear / depth;
}
...
int R_SpriteFogNum( trRefEntity_t *ent ) {
	...
	if ( ent->ePtr->renderfx & RF_CROSSHAIR ) {
		return 0;
	}
...
void R_SortDrawSurfs( drawSurf_t *drawSurfs, int numDrawSurfs ) {
	...
	if ( numDrawSurfs > MAX_DRAWSURFS ) {
		Com_Printf("^3numDrawSurfs > MAX_DRAWSURFS (%d)\n", MAX_DRAWSURFS);
