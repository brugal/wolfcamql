...
static qboolean R_LoadMD3 (model_t *mod, int lod, void *buffer, const char *mod_name ) {
	...
	for ( i = 0 ; i < mod->md3[lod]->numSurfaces ; i++) {
		...
		if ( surf->numVerts > SHADER_MAX_VERTEXES ) {
			ri.Error (ERR_DROP, "R_LoadMD3: %s has more than %i verts on %s (%i)",
					  mod_name, SHADER_MAX_VERTEXES, surf->name[0] ? surf->name : "a surface", surf->numVerts );
		}
		if ( surf->numTriangles*3 > SHADER_MAX_INDEXES ) {
			ri.Error (ERR_DROP, "R_LoadMD3: %s has more than %i triangles on %s (%i)",
					  mod_name, SHADER_MAX_INDEXES / 3, surf->name[0] ? surf->name : "a surface", surf->numTriangles );
		}
