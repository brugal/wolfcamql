...
#if 0
	if (*r_mapColor->string) {
		int v;

		v = r_mapColor->integer;
		out[0] = (v & 0xff0000) / 0x010000;
		out[1] = (v & 0x00ff00) / 0x000100;
		out[2] = (v & 0x0000ff) / 0x000001;
		out[3] = in[3];
...
static	void R_LoadLightmaps( lump_t *l ) {
	...
	if ( r_vertexLight->integer || glConfig.hardwareType == GLHW_PERMEDIA2 ) {
		// wc don't return to avoid crashing with r_vertexLight 1 and /devmap
		//return;
	}
	...
	for ( i = 0 ; i < tr.numLightmaps ; i++ ) {
		...
		{	// color code by intensity as development tool	(FIXME: check range)
		...
		} else if (r_lightmap->integer == 4) {
			float v;
			int r, g, b;

			v = r_mapOverBrightBits->value;
			for (j = 0;  j < LIGHTMAP_SIZE * LIGHTMAP_SIZE;  j++) {
				r = (float)buf_p[j * 3 + 0] * v;
				g = (float)buf_p[j * 3 + 1] * v;
				b = (float)buf_p[j * 3 + 2] * v;
			...
			R_GammaCorrectLightMap(image, LIGHTMAP_SIZE*LIGHTMAP_SIZE*4);
		} else
#endif
{
			for ( j = 0 ; j < LIGHTMAP_SIZE * LIGHTMAP_SIZE; j++ ) {
				...
				// 2017-11-22  wc: 1137:45c87181ed63 had set to 128 because of transparent ads not working, changing back to original 255 value
				image[j*4+3] = 255;
...
#if 0
		if (!(out[i].surfaceFlags & SURF_SKY)  &&  (r_singleShader->integer == 2  ||  r_singleShader->integer == 4)  &&  *r_singleShaderName->string) {
			Q_strncpyz(out[i].shader, r_singleShaderName->string, sizeof(out[i].shader));
		}
#endif
#if 0
...
static	void R_LoadFogs( lump_t *l, lump_t *brushesLump, lump_t *sidesLump ) {
	...
	for ( i=0 ; i<count ; i++, fogs++) {
		...
		if ( (unsigned)out->originalBrushNumber >= brushesCount ) {
			ri.Error( ERR_DROP, "fog brushNumber out of range %d >= %d", (unsigned)out->originalBrushNumber, brushesCount );
		}
...
void RE_LoadWorldMap( const char *name ) {
	...
	if ( i > BSP_VERSION ) {  //FIXME only check supported ones
