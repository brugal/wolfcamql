...
static void CG_DamageFeedback( int yawByte, int pitchByte, int damage ) {
	...
	if (cg.snap->serverTime - cg.v_dmg_time > cg_damageFeedbackInterval.integer) {
		cg.v_dmg_time = cg.snap->serverTime;
	} else {
		//Com_Printf("skipping view damage %d\n", cg.snap->serverTime);
	}
}
...
void CG_TransitionPlayerState( const playerState_t *ps, playerState_t *ops ) {
	...
	if (cg.demoPlayback) {
		if (ps->weapon != ops->weapon) {
			...
			wclients[ps->clientNum].damagePlumSum = 0;
			wclients[ps->clientNum].damagePlumTime = 0;
