...
#define GLE( ret, name, ... ) qgl##name = (name##proc *) SDL_GL_GetProcAddress("gl" #name); \
	...
	if ( Q_stricmpn( "OpenGL ES", version, 9 ) == 0 ) {
		...
		if (sscanf( version, "%d.%d", &qglMajorVersion, &qglMinorVersion ) != 2) {
			ri.Printf(PRINT_WARNING, "couldn't parse opengl major and minor version\n");
			//FIXME set to 0 ?
		}
	}
...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder, qboolean fixedFunction)
{
	...
	if( SDL_window != NULL )
	{
		...
		SDL_GetWindowPosition( SDL_window, &oldX, &oldY );
	...
	for (i = 0; i < 16; i++)
	{
...
#if 0 // If not allowing software GL, demand accelerated
		...
		if (!fixedFunction  &&  r_useCoreContext->integer)
		{
...
static void GLimp_InitExtensions( qboolean fixedFunction )
{
	...
	if (SDL_GL_ExtensionSupported("GL_EXT_framebuffer_object")) {
		...
		if (r_fboStencil->integer) {
			if (SDL_GL_ExtensionSupported("GL_EXT_packed_depth_stencil")) {
		...
		if (SDL_GL_ExtensionSupported("GL_EXT_framebuffer_multisample")  &&  SDL_GL_ExtensionSupported("GL_EXT_framebuffer_blit")) {
