...
static qboolean RawImage_ScaleToPower2( byte **data, int *inout_width, int *inout_height, imgType_t type, imgFlags_t flags, byte **resampledBuffer)
{
	...
	//FIXME should also check *data and *resampledBuffer here and in the other references
	     ...
	     scaled_width < r_imageUpsampleMaxSize->integer && scaled_height < r_imageUpsampleMaxSize->integer)
	{
		...
		*resampledBuffer = ri.Hunk_AllocateTempMemory( finalwidth * finalheight * 4 );
		...
		if (r_debugScaledImages->integer) {
			int i;
			unsigned char *sd = (unsigned char *)*resampledBuffer;
		...
		if (type == IMGTYPE_COLORALPHA)
		{
			if( r_greyscale->integer )
			{
				for ( i = 0; i < c; i++ )
				{
					...
					luma = (float)luma * r_greyscaleValue->value;
					scan[i*4] = luma;
					scan[i*4 + 1] = luma;
					scan[i*4 + 2] = luma;
				}
...
================
*/
image_t *R_CreateImage2( const char *name, byte *pic, int width, int height, GLenum picFormat, int numMips, imgType_t type, imgFlags_t flags, int internalFormat, image_t *imageIn ) {
	byte       *resampledBuffer = NULL;
	image_t    *image;
	qboolean    isLightmap = qfalse, scaled = qfalse;
	//qboolean	isFatLightmap = qfalse;
	long        hash;
	...
	if ( !strncmp( name, "*lightmap", 9 ) ) {
		//ri.Printf(PRINT_ALL, "^2 '%s' is lightmap %p\n", name, pic);
		isLightmap = qtrue;
	}
#if 0
	if ( !strncmp( name, "_fatlightmap", 12 ) ) {
		//ri.Printf(PRINT_ALL, "^5 '%s' is fatlightmap %p\n", name, pic);
		isFatLightmap = qtrue;
	}
#endif
	...
	if (imageIn != NULL) {
		//ri.Printf(PRINT_ALL, "start: '%s'  rgba8:%d  %dx%d\n", name, rgba8, width, height);
	}
	...
	if (imageIn == NULL) {
		...
		qglDeleteTextures(1, &imageIn->texnum);
		qglGenTextures(1, &imageIn->texnum);
		image = imageIn;
	}
	...
	// GL_RGBA8 is for regular lightmaps
	if (pic != NULL  &&  internalFormat == GL_RGBA8) {

		//FIXME fat lightmaps
		//if (*r_lightmapColor->string  &&  (isLightmap ||  isFatLightmap)) {
...
#if 0
		if (picFormat == GL_RGBA8  &&  pic != NULL) {
	...
	{
		if (rgba8) {
			if (pic) {  // fbo_create NULL image check
...
{
	//ri.Printf(PRINT_ALL, "create image: '%s' %d %d\n", name, width, height);
...
void R_CreateBuiltinImages( void ) {
	...
	if (r_imageUpsample->integer) {
		screenMapTextureSize /= 2;
	}
...
void R_CreatePlayerColorSkinImages (qboolean force)
{
	...
	for (i = 0;  Skin_Images[i][0];  i++) {
		...
		// get red
		if (Skin_Images[i][1]) {
			R_LoadImage(Skin_Images[i][1], &rpic, &rw, &rh, &picFormat, &picNumMips);
		} else {
			R_LoadImage(va(Skin_Images[i][0] + strlen("cache/"), "red", ""), &rpic, &rw, &rh, &picFormat, &picNumMips);
		}
		...
		if (Skin_Images[i][2]) {
			R_LoadImage(Skin_Images[i][2], &bpic, &bw, &bh, &picFormat, &picNumMips);
		} else {
			R_LoadImage(va(Skin_Images[i][0] + strlen("cache/"), "blue", ""), &bpic, &bw, &bh, &picFormat, &picNumMips);
		}
