...
static qboolean UI_GameType_HandleKey(int flags, float *special, int key, qboolean resetMap) {
	...
	if (select != 0) {
		...
		//FIXME 2016-07-13 wolfcam gametypes
		if (uiInfo.gameTypes[ui_gameType.integer].gtEnum < GT_TEAM) {
...
static void UI_RunMenuScript(char **args) {
	...
	if (String_Parse(args, &name)) {
		if (Q_stricmp(name, "StartServer") == 0) {
			...
			//FIXME hack to override default cfg
...
void _UI_SetActiveMenu( uiMenuCommand_t menu ) {
	...
	// ensure minumum menu data is cached
  if (Menu_Count() > 0) {
		vec3_t v;
		v[0] = v[1] = v[2] = 0;
	  switch ( menu ) {
...
static cvarTable_t		cvarTable[] = {
	...
	{ &ui_redteam, "ui_redteam", DEFAULT_BLUETEAM_NAME, CVAR_ARCHIVE },
	{ &ui_blueteam, "ui_blueteam", DEFAULT_REDTEAM_NAME, CVAR_ARCHIVE },
