...
static void FS_CopyFile( char *fromOSPath, char *toOSPath ) {
	...
	buf = malloc( len );
	if (!buf) {
		Com_Error(ERR_FATAL, "FS_CopyFile couldn't allocate memory %ld", (long)len);
	}
...
int	FS_GetModList( char *listbuf, int bufsize ) {
	...
	*listbuf = 0;
	...
	for (i = 0; i < nPotential; i++) {
		...
		if (bDrop || /*Q_stricmp(name, com_basegame->string) == 0*/ Q_stricmp(name, BASEGAME) == 0 || Q_stricmpn(name, ".", 1) == 0) {
			continue;
		}
