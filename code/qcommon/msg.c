...
void MSG_Copy(msg_t *buf, byte *data, int length, msg_t *src)
{
	if (length<src->cursize) {
		MSG_Error( ERR_DROP, "MSG_Copy: can't copy into a smaller msg_t buffer");
		return;
	}
...
//	FILE*	fp;
	...
	if ( bits == 0 || bits < -31 || bits > 32 ) {
		MSG_Error( ERR_DROP, "MSG_WriteBits: bad bits %i", bits );
		return;
	}
...
#ifdef PARANOID
	if (c < -128 || c > 127) {
		MSG_Error (ERR_FATAL, "MSG_WriteChar: range error");
		return;
	}
#endif
...
#ifdef PARANOID
	if (c < 0 || c > 255) {
		MSG_Error (ERR_FATAL, "MSG_WriteByte: range error");
		return;
	}
#endif
...
void MSG_WriteShort( msg_t *sb, int c ) {
	...
	if (c < ((short)0x8000) || c > (short)0x7fff) {
		MSG_Error (ERR_FATAL, "MSG_WriteShort: range error");
		return;
	}
#endif
...
void MSG_WriteDeltaEntity( msg_t *msg, struct entityState_s *from, struct entityState_s *to, 
	...
	if ( to->number < 0 || to->number >= MAX_GENTITIES ) {
		MSG_Error (ERR_FATAL, "MSG_WriteDeltaEntity: Bad entity number: %i", to->number );
		return;
	}
