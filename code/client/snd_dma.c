...
static void S_SpatializeOrigin (const vec3_t origin, int master_vol, int *left_vol, int *right_vol)
{
	...
	//Com_Printf("dist %f  %f\n", dist, Distance(listener_origin, origin));

	if (!s_qlAttenuate->integer) {  // old q3 code
...
static int LoopSoundsPresent[MAX_LOOP_SOUNDS];
...
============
*/
static void S_Base_RawSamples( int stream, int samples, int rate, int width, int numChannels, const byte *data, float volume, int entityNum) {
	int		i;
	int		src, dst;
	float	scale;
...
static void S_OpenBackgroundStream( const char *filename ) {
	...
	if(s_backgroundStream->info.channels != 2 || s_backgroundStream->info.rate != 22050) {
		Com_Printf(S_COLOR_YELLOW "WARNING: music file %s is not 22k stereo and/or channels != 2\n", filename );
