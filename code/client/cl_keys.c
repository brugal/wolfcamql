...
void Field_VariableSizeDraw (field_t *edit, float x, float y, int width, int size, qboolean drawSmall, float cwidth, float cheight, qboolean showCursor, qboolean noColorEscape)
{
	...
	if ( showCursor ) {
		...
		//if ( (int)( cls.realFrametime >> 8 ) & 1 ) {
		if ( (int)( Sys_Milliseconds() >> 8 ) & 1 ) {
			return;		// off blink
		}
...
{
	Field_VariableSizeDraw( edit, x, y, width, SMALLCHAR_WIDTH, qtrue, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, showCursor, noColorEscape );
}

void Field_BigDraw( field_t *edit, int x, int y, int width, qboolean showCursor, qboolean noColorEscape )
{
	Field_VariableSizeDraw( edit, x, y, width, BIGCHAR_WIDTH, qfalse, BIGCHAR_WIDTH, BIGCHAR_HEIGHT, showCursor, noColorEscape );
}
...
void Field_KeyDownEvent( field_t *edit, int key ) {
	...
	if ( ( ( key == K_INS ) || ( key == K_KP_INS  &&  !cl_numberPadInput->integer) ) && keys[K_SHIFT].down ) {
		Field_Paste( edit );
		return;
	}
...
void Console_Key (int key)
{
	...
	if ( (key == K_MWHEELUP && keys[K_SHIFT].down) || ( key == K_UPARROW ) || ( key == K_KP_UPARROW  &&  !cl_numberPadInput->integer) ||
	...
	if ( (key == K_MWHEELDOWN && keys[K_SHIFT].down) || ( key == K_DOWNARROW ) || ( key == K_KP_DOWNARROW  &&  !cl_numberPadInput->integer) ||
...
void Message_Key( int key ) {
	...
	if ( key == K_ENTER || key == K_KP_ENTER )
	{
		if ( chatField.xbuffer[0].codePoint && clc.state == CA_ACTIVE ) {
			if (chat_playerNum != -1 )
...
void Cgame_Key_GetBinding (int keynum, char *buffer)
{
	...
	if (keys[keynum].binding  &&  *keys[keynum].binding) {
		Q_strncpyz(buffer, keys[keynum].binding, MAX_STRING_CHARS);
	} else {
		buffer[0] = '\0';
	}
}
...
void CL_KeyDownEvent( int key, unsigned time )
{
	...
	if( key == K_CONSOLE ||  key == K_F1  || ( keys[K_SHIFT].down && key == K_ESCAPE ) )
	{
		Con_ToggleConsole_f ();
		Key_ClearStates ();
		return;
	...
	if ( ( key < 128 || key == K_MOUSE1 ) &&
		 ( (clc.demoplaying  &&  (clc.state != CA_DOWNLOADINGWORKSHOPS  &&  clc.state != CA_LOADING)) || clc.state == CA_CINEMATIC ) && Key_GetCatcher( ) == 0 ) {
